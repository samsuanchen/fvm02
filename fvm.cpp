...
void FVM::flush(){        // flush out from terminal out buffer to console
	Serial.print(T->tob);
	T->oEnd = T->tob;
//	PRINTF("before appending T->html=0x%x\n", T->html); while(1);
	if(T->html){
		PRINTF("before appending T->html=0x%x\n", T->html); while(1);
		PRINTF("before appending (*T->html).length()=%d\n", (*T->html).length());
		PRINTF("before appending *T->html=\"%s\"\n", (*T->html).c_str());
		*T->html = String(*T->html + "\r\n" + String(T->tob));
		PRINTF("after appending *T->html=\"%s\"\n", (*T->html).c_str());
		_send();
	}
}
...
void	FVM::dotId( Word*w ){ // show word's id and name
  print('W'); printHexZ(w->id, 3); print(' 0x'); printHex(*w->name); print(w->name+1); print(' ');
}
/////////////////////////////////////////////////////////////////////////////////////////////////////
// FVM function 09 show return stack and data stack
...
void    FVM::dPush     (int n) { if(! dHasSpace(1)){ abort(-1, "dPush over"); return; }
		*(++(T->DP))=n; } // push integer onto stack
int     FVM::dPop      ()      { if(dDepth()<1){ abort(-2, "dPop under"); return 0; }
		return *(T->DP--); } // pop integer from stack
int     FVM::dPick     (int i) { if(! dHasItems(i+1)){ abort(-3, "dPick under"); return 0; }
		return *(T->DP-i) ; } // top i-th integer of stack
void    FVM::dRoll     (int n) { if(! dHasItems(n+1)){ abort(-3, "dRoll under"); return; }
		int *p=T->DP-n, X=*p; while(++p <= T->DP) *(p-1)=*p; *(p-1)=X; } // roll top n integers on stack
void    FVM::dBackRoll (int n) { if(! dHasItems(n+1)){ abort(-3, "dBackRoll under"); return; }
...
Task * FVM::createTask( int sizeTob, int sizeTib, int sizeDS, int sizeRS, int sizeCS ){
  int bytesForTask = sizeof(Task), bytesForTob = sizeTob, bytesForTib = sizeTib;
  int bytesForDS = sizeDS*sizeof(int), bytesForRS = sizeRS*sizeof(int), bytesForCS = sizeCS*4;
  int bytesTotal = bytesForTask + bytesForTob + bytesForTib + bytesForDS + bytesForRS + bytesForCS;
...
/////////////////////////////////////////////////////////////////////////////////////////////////////
void 	FVM::initCall(){ ipPush(), T->IP = T->W->p.wpl; }
/////////////////////////////////////////////////////////////////////////////////////////////////////
void    FVM::ms(int n){ if(! T->waitMsUntil) T->waitMsUntil=millis(); T->waitMsUntil += n; }
// set to wait give time period (milli seconds)
/////////////////////////////////////////////////////////////////////////////////////////////////////
void    FVM::callPerWrd () { // execute word one by one in the wplist of a colon word
  ...
  if ( ! (T->IP) ){
  //print("\ncallPerWrd() end of full calling");
...
void    FVM::initEval (char *str) {
  ...
  if ( T->tracing )
    print("\n000 initEval 0x"), printHex(n), print(" \""), print(T->pBgn), print("\"\n");
}
/////////////////////////////////////////////////////////////////////////////////////////////////////
void    FVM::parsPerTkn() { // parse token to evaluate
  if( millis() <= T->waitMsUntil ) return; // wait up to given time
  if( T->state == 0 ) return;
  if( T->IP )       return;
  if( *(T->pEnd)==0 ) { initRead(); return; }  // end of parsing
...
//tracing=1;
  ...
  if ( T->W ) {                                  // found word W
    ...
    if( (flag&COMPO)&&((T->state) & CMPLING)==0 ) { 
	  abort( 6, "cannot execute compile-only type word"); return; }
      if( T->tracing ) print( "execute " ), showWordType(T->W);
      T->W->code();							  // execute word W
    }
    else {
      if( T->tracing ) print( "compile " ), showWordType(T->W);
  ...
  if( T->tracing ) print("0x"), printHex(n), print(' '), print(n), print(' ');
}
/////////////////////////////////////////////////////////////////////////////////////////////////////
char*   FVM::parseToken (char delimiter){ // parse token by given delimiter (if delimiter=0x20, take white space as delimiter)
  ...
  if( T->tracing ) print('\n'), printHexZ(T->tokenAt-T->pBgn,3), print(" parsPerTkn 0x"), printHex(n), print(" \""), print(&tmp[1]), print("\" ");
...
/////////////////////////////////////////////////////////////////////////////////////////////////////
void    FVM::initRead() { // set to input char from serial port
...
//}
  if ( c == '\r' ) { // carriage return or line feed
    *(T->iEnd) = 0; // append end of input
    initEval(T->tib); return;        // interpret tib
  }
if ( T->iEnd >= T->tib+TIB_SIZE-1 ) { abort( 9, "tib full of TIB_SIZE" ); return; }
...
uint64_t times=0;
void FVM::update(){
	if(times++ == 0) cr(), showTime(), print(" first run F.update()"), flush();
	readPerChr(), parsPerTkn(), callPerWrd(); }
/////////////////////////////////////////////////////////////////////////////////////////////////////
...
/////////////////////////////////////////////////////////////////////////////////////////////////////
//String html;
void FVM::init( long baud, int cpu ){
...
#endif
  Serial.begin( baud );
  sbEnd=SB; T = createTask(); *T->tob=0; T->html=0;
  print("ESP32 Chip id = "), printHex(ESP.getEfuseMac()), cr();
  showTime(), print(" run F.init()"), cr();
//String html="hello";
//T->html=&html;
//PRINTF("html=0x%x, T->html=0x%x, *T->html=0x%x, (*T->html).length()=0x%x\n", html, T->html, *T->html, (*T->html).length());
//*T->html=String(html+" T->tob --> "+String(T->tob));
//PRINTF("html=0x%x, T->html=0x%x, *T->html=0x%x, (*T->html).length()=0x%x\n", html, T->html, *T->html, (*T->html).length());
//PRINTF("T->html=0x%x\n", T->html); while(1);
//*T->html = String("");
//PRINTF("T->html=0x%x T->html.length()=0x%x",T->html,T->html.length()); while(1);
  T->tracing = 0;
  delay(100);
...
//if(cpu){
	  extern void xxx(int);
	  xxx(cpu); // run F.update per ms at specified CPU
//}
  if(strstr(word_set_logo,"wifiboy_lib")) wb_init(1);
}
/////////////////////////////////////////////////////////////////////////////////////////////////////